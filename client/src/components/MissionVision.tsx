    ...
    >
      ...
      <div className="container mx-auto px-4 lg:px-6">
        <div className="max-w-6xl mx-auto">
          <div className={`text-center mb-16 ${mvVisible ? 'animate-fade-in' : 'opacity-0'}`}>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-6 text-gradient" data-testid="text-mission-vision-title">
              Mission & Vision
            </h2>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto" data-testid="text-mission-vision-subtitle">
              Our commitment to educational institutions and institutional partnership excellence
            </p>
          </div>

          <div className="grid lg:grid-cols-2 gap-12">
            {/* Mission */}
            <div className={`${mvVisible ? 'animate-slide-in-left' : 'opacity-0'}`}>
              <div className="bg-card rounded-2xl p-8 shadow-lg card-hover h-full" data-testid="card-mission">
                <div className="flex items-center mb-6">
                  ...
                  <h3 className="text-2xl font-bold text-card-foreground" data-testid="text-mission-title">Partnership Mission</h3>
                </div>
                <ul className="space-y-4" data-testid="list-mission">
                  {missionPoints.map((point, index) => (
                    <li key={index} className="flex items-start" data-testid={`mission-point-${index + 1}`}>
            ...
            {/* Vision */}
            <div className={`${mvVisible ? 'animate-slide-in-right' : 'opacity-0'}`}>
              <div className="bg-card rounded-2xl p-8 shadow-lg card-hover h-full" data-testid="card-vision">
                <div className="flex items-center mb-6">
                  ...
                  <h3 className="text-2xl font-bold text-card-foreground" data-testid="text-vision-title">Partnership Vision</h3>
                </div>
                <ul className="space-y-4" data-testid="list-vision">
                  {visionPoints.map((point, index) => (
                    <li key={index} className="flex items-start" data-testid={`vision-point-${index + 1}`}>
