...
import { Star, Crown, Calendar, Users, Briefcase, ClipboardList, Check } from 'lucide-react';
  ...
  const teamMembers = [
    {
      ...
      skillColors: ["bg-primary text-primary-foreground", "bg-secondary text-secondary-foreground", "bg-accent text-accent-foreground", "bg-primary text-primary-foreground", "bg-secondary text-secondary-foreground"],
      bgColor: "primary",
      accentColor: "primary"
    },
    {
      ...
      skillColors: ["bg-secondary text-secondary-foreground", "bg-accent text-accent-foreground", "bg-primary text-primary-foreground"],
      bgColor: "secondary",
    ...
    >
      ...
      <div className="absolute inset-0 opacity-5">
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          ...
          <rect width="100%" height="100%" fill="url(#team-grid)" className="text-foreground" />
        </svg>
      </div>
      
      <div className="relative z-10 container mx-auto px-4 lg:px-6">
        <div className="max-w-7xl mx-auto">
          <div className={`text-center mb-16 ${teamVisible ? 'animate-fade-in' : 'opacity-0'}`}>
            ...
            <h2 className="text-4xl md:text-5xl font-bold mb-6 text-foreground" data-testid="text-team-title">
              Institutional <span className="text-primary">Training</span> <span className="text-secondary">Specialists</span>
            </h2>
            <p className="text-lg text-muted-foreground max-w-3xl mx-auto leading-relaxed" data-testid="text-team-subtitle">
              Experienced professionals dedicated to enhancing placement success rates for educational institutions across India
            </p>
          </div>
          ...
          <div className="space-y-20">
            {teamMembers.map((member, index) => (
              <div 
                key={index}
                className={`${teamVisible ? (index === 0 ? 'animate-slide-in-left' : 'animate-slide-in-right') : 'opacity-0'}`}
                data-testid={`team-member-${index + 1}`}
              >
                <div className={`grid lg:grid-cols-5 gap-8 items-center ${index % 2 === 0 ? '' : 'lg:grid-flow-col-dense'}`}>
                  {/* Professional Profile Image */}
                  <div className={`lg:col-span-2 ${index % 2 === 0 ? '' : 'lg:order-last'} relative`}>
                    <div className="relative overflow-hidden rounded-xl shadow-lg border border-border">
                      ...
                      <div className="absolute top-4 left-4">
                        <div className={`${
                          member.bgColor === 'primary' ? 'bg-primary text-primary-foreground' :
                          'bg-secondary text-secondary-foreground'
                        } p-3 rounded-lg shadow-lg`}>
                          <div className="text-lg font-bold">{member.experience.split(' ')[0]}</div>
                  ...
                  <div className="lg:col-span-3 space-y-8">
                    ...
                    <div className="space-y-4">
                      <div className={`inline-flex items-center px-4 py-2 rounded-full font-medium shadow-sm border ${
                        member.bgColor === 'primary' ? 'bg-primary/10 text-primary border-primary/20' :
                        'bg-secondary/10 text-secondary border-secondary/20'
                      }`}>
                        {index === 0 ? <Star className="w-4 h-4 mr-2" /> : <Crown className="w-4 h-4 mr-2" />}
                        <span data-testid={`text-team-role-${index + 1}`}>{member.role}</span>
                      </div>
                      <h3 className="text-3xl md:text-4xl font-bold text-foreground leading-tight" data-testid={`text-team-name-${index + 1}`}>
                        {member.name}
                      </h3>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-card rounded-lg p-4 border border-border shadow-sm">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <div className="text-foreground font-semibold text-sm" data-testid={`text-team-experience-${index + 1}`}>{member.experience}</div>
                            <div className="text-muted-foreground text-xs">Teaching Excellence</div>
                          </div>
                        </div>
                      </div>
                      <div className="bg-card rounded-lg p-4 border border-border shadow-sm">
                        <div className="flex items-center space-x-3">
                          <div className="w-10 h-10 bg-secondary rounded-lg flex items-center justify-center">
                            {index === 0 ? <Users className="w-5 h-5 text-secondary-foreground" /> : <Briefcase className="w-5 h-5 text-secondary-foreground" />}
                          </div>
                          <div>
                            <div className="text-foreground font-semibold text-xs leading-tight" data-testid={`text-team-achievement-${index + 1}`}>{member.studentsTrained}</div>
                            <div className="text-muted-foreground text-xs">Impact & Results</div>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      {member.description.map((paragraph, pIndex) => (
                        <p 
                          ...
                          data-testid={`text-team-description-${index + 1}-${pIndex + 1}`}
                        >
                          {paragraph}
                        </p>
                      ))}
                    ...
                    {member.responsibilities && (
                      <div className="bg-card rounded-lg p-6 border border-border shadow-sm">
                        <h4 className="text-foreground font-bold mb-4 flex items-center text-base">
                          <ClipboardList className="w-5 h-5 text-accent mr-3" />
                          Key Responsibilities
                        </h4>
                        <ul className="space-y-3" data-testid={`list-team-responsibilities-${index + 1}`}>
                          {member.responsibilities.map((responsibility, rIndex) => (
                            <li key={rIndex} className="flex items-start text-muted-foreground">
                    ...
                    {/* Skills */}
                    <div className="flex flex-wrap gap-3" data-testid={`skills-team-member-${index + 1}`}>
          ...
          <div className={`mt-16 ${teamVisible ? 'animate-fade-in' : 'opacity-0'}`} style={{animationDelay: '0.8s'}}>
            <div className="bg-card rounded-xl p-8 md:p-12 border border-border shadow-lg" data-testid="team-stats-card">
              <h3 className="text-2xl md:text-3xl font-bold mb-8 text-center text-foreground" data-testid="text-team-stats-title">
                Proven <span className="text-primary">Partnership</span> <span className="text-secondary">Success</span>
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="text-center" data-testid="stat-combined-experience">
                  <div className="w-16 h-16 bg-primary rounded-lg flex items-center justify-center mx-auto mb-4">
                    <Calendar className="w-8 h-8 text-primary-foreground" />
                  </div>
                  <div className="text-3xl font-bold mb-2 text-foreground">25+</div>
                  <div className="text-muted-foreground font-medium">Years Combined Experience</div>
                </div>
                <div className="text-center" data-testid="stat-students-impacted">
                  <div className="w-16 h-16 bg-secondary rounded-lg flex items-center justify-center mx-auto mb-4">
                    <Users className="w-8 h-8 text-secondary-foreground" />
                  </div>
                  <div className="text-3xl font-bold mb-2 text-foreground">200K+</div>
                  <div className="text-muted-foreground font-medium">Students Trained via College Partnerships</div>
                </div>
                <div className="text-center" data-testid="stat-dedication">
                  ...
                  <div className="text-muted-foreground font-medium">Institutional Partner Satisfaction</div>
                </div>
              </div>
            </div>
          </div>
